...
renderPlot({
  ...
  fac <- c(rep("antes", 1000), rep("después", length(Zb)))
...
renderPlot({
  H2 <- input$SOB
  Z2 <- rnorm(1000)
  Zb2 <- Z2[Z2 > quantile(Z2, probs = H2)]
  Y2 <- rep(0, length(Z2) + length(Zb2))
  fac2 <- c(rep("antes", 1000), rep("después", length(Zb2)))
  dat2 <- data.frame(ind = c(Z2, Zb2), pres = fac2, Y = Y2)
  o <- order(dat2$ind)
  m <- qplot(ind, ..count.., data= dat2, geom="density", fill=pres, adjust = 4) 
  m <- m + geom_vline(xintercept = 0) + geom_vline(xintercept = mean(Zb2))
  m <- m + xlim(-4, 4) + theme_bw() 
  m <- m + theme(legend.position="top", axis.text.y=element_blank(), 
                 axis.title.y = element_blank(), axis.title.x = element_blank(),
                 legend.title = element_blank()) 
  ...
  n  <- n + geom_segment(aes(x = 0, y = 0, xend = mean(Zb2) , yend = 0, size=4), color ="black") 
  n <- n + xlim(-4, 4) + theme_bw()
  n <- n + theme(legend.position="none", axis.title.y=element_blank(), 
                 axis.text.y=element_blank()) 
...
***
  selectInput("SEL", label = h3("selección"), choices = list("Disruptiva" = 0, "Estabilizadora" = 1), selected = 0)